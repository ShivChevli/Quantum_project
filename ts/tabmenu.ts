export class TabMenu{
    ...
    tab_items: Array<HTMLElement>
    tab_list_length: number;
    ...
    onTabItemKeydown(event: KeyboardEvent) {
        let key = event.key;
        let flag = false;
    ...
    changeTab(new_active_tab:HTMLElement=undefined) {
        if (new_active_tab === undefined) {
            new_active_tab = this.tab_items[this.active_tab_index];
        }
        this.tab_items.forEach((el: HTMLElement) => {
    ...
    activeTab(new_active_tab:HTMLElement = undefined) {
        if (new_active_tab === undefined) {
            new_active_tab = this.tab_items[this.active_tab_index];
        }
